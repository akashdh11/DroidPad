package com.akash.hiremath.droidpad.keyboardUtils

import android.bluetooth.BluetoothDevice
import android.bluetooth.BluetoothHidDevice
import android.util.Log
import android.view.KeyEvent
import com.akash.hiremath.droidpad.DescriptorCollection
...
open class KeyboardSender(
    ...
    val keyboardReport:KeyboardReport = KeyboardReport()
    //val keyPosition : IntArray = IntArray(6){0}



    protected open fun sendKeys() {
        if (!hidDevice.sendReport(host, DescriptorCollection.KEYBOARD_ID, keyboardReport.bytes)) {
    ...
    fun setLeftCtrlOn(){
        keyboardReport.leftControl = true
    }
    fun setLeftAltOn(){
